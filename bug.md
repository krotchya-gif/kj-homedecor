...
- **Route API tanpa caller produksi:** `api/customers`, `api/landing-settings`, `api/materials`, `api/products`, `api/suppliers`, `api/purchase-orders` (+`[id]`), `api/purchase-requests` (+`[id]`), `api/install-bookings` (base), `api/orders` (base — hanya dipakai E2E smoke GET 403). **Fase 1 (2026-08-13): semua GET route ini sudah di-role-gate** → kini berfungsi sebagai lapisan API hardening (defense-in-depth) walau tidak dipanggil UI; keputusan owner: tetap dipertahankan.
...
- **Export dead:** `clientError` (`src/lib/api-errors.ts:14`) — **DIHAPUS** (2026-08-13, sesi 34: nol referensi).
