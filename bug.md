...
| BUG-079 | **Search pesanan tidak menampilkan apa-apa** — `query.or('...customer.name.ilike...')` → PostgREST `.or()` tidak mendukung kolom tabel relasi → query error diam-diam → UI kosong walau data ada | ✅ Fixed (2026-08-13, migration 081) | RPC `search_orders` filter di SQL (orders + `EXISTS customers.name` + status + kategori), return `{rows,total}`; `fetchOrders` pakai RPC + tampilkan error. Verifikasi UI: "25" → 6 baris |
| BUG-080 | **Task laundry tak bisa diterima** — check constraint `laundry_orders_status_check` di live hanya `('pending','done','cancelled')` tanpa `'in_progress'` (drift) → update `pending→in_progress` (tombol "Terima Task") gagal 23514 | ✅ Fixed (2026-08-13, migration 082) | Drop + recreate constraint `('pending','in_progress','done','cancelled')` + sync `000_full_schema.sql`. Diverifikasi simulasi: terima → lapor selesai → payroll → jurnal |
| BUG-081 | **RLS `landing_settings` terbuka** — policy write `FOR ALL USING (auth.role()='authenticated')` → SEMUA staff login (penjahit/surveyor/laundry) bisa ubah konten landing & SEO; halaman admin menulis via client SDK (bukan `/api/landing-settings` yang admin-only) sehingga validasi API tidak pernah aktif | ✅ Fixed (2026-08-13, migration 083) | Policy write → `is_admin_or_owner_sd()` (SELECT publik tetap). Diverifikasi helper tersedia untuk authenticated |
| BUG-082 | **Upload sitemap/robots tampil "Upload failed" padahal sukses** — `seo/page.tsx` cek `data?.success` tapi API mengembalikan `{ data: { path }, error: null }` tanpa field `success` → UI selalu error | ✅ Fixed (2026-08-13) | Kontrak respons API → `{ success: true, data: { path } }` (klien cek `data?.success`). Isi file kini disimpan ke DB (`sitemap_content`/`robots_content`) bukan filesystem — persist saat redeploy (route `/sitemap.xml` & `/robots.txt` baca dari DB) |
| BUG-083 | **Trust badges diedit admin tapi tidak pernah tampil di landing publik** — `trust_badges` hanya dirender di preview admin; `TRUST_ICON_MAP` di `page.tsx` tak terpakai | ✅ Fixed (2026-08-13) | `ScrollHero` terima prop `trustBadges` dari DB (fallback angka hardcoded); di-render di hero stats |
| BUG-084 | **Preset tema `modern` berisi CSS-var** (`var(--neutral-800)` dll) bukan hex → ColorPicker invalid, preview & landing menghasilkan CSS rusak (`var(...)dd` hex-alpha) | ✅ Fixed (2026-08-13) | Preset `modern` → hex nyata (`#1f2937`/`#374151`/`#3b82f6`/`#f3f4f6`/`#111827`) |
| BUG-085 | **`/owner/staff` kosong** — `select('*, order_logs(count)')` ambigu (2 FK `users↔order_logs`: `created_by`+`staff_id`) → PGRST201 HTTP 300 → data null; plus kolom Email kosong (tidak ada di `public.users`) + urutan role acak (`laundry`/`surveyor` terangkat ke atas karena `indexOf`=-1) | ✅ Fixed (2026-08-13) | Query → `select('*')` (agregat tak dipakai); kolom Email dihapus dari UI; `ROLE_ORDER` lengkap 8 role + badge label role/status; tombol aksi jelas "Kelola Staff" → `/admin/staff` |
| BUG-086 | **GET `orders/[id]` tanpa role gate** — role rendah (penjahit/surveyor/installer) bisa baca PII pelanggan (`customer(*)` phone/address) order apa pun; inkonsisten dgn GET koleksi yang sudah dibatasi | ✅ Fixed (Phase 1, 2026-08-13) | Role-gate server-side: `admin/owner/finance/gudang` + `status='active'` (pola `orders/route.ts`). Alasan metode: klien bisa di-bypass → gate di server; konsisten dgn GET koleksi |
| BUG-087 | **GET `install-bookings` & `[id]` tanpa role/ownership** — PII customer (phone/address) + detail order terbaca semua role login | ✅ Fixed (Phase 1, 2026-08-13) | GET koleksi: admin/owner/finance = semua; installer = hanya miliknya. GET `[id]`: mirror PUT (ownership check installer). Alasan: PII tidak boleh bocor; konsisten dgn PUT |
| BUG-088 | **Komentar `admin/staff` "RLS users masih terbuka"** — menyesatkan: klaim tersebut sudah usang (write users sudah dikunci admin/owner via `is_admin_or_owner_sd()`, migration 072/078) → agent berikutnya bisa bikin migration redundan | ✅ Fixed (Phase 1, 2026-08-13) | Perbaiki komentar: guard client = defense-in-depth, BUKAN compensating control utk RLS terbuka. Alasan (SOP #1/#2): verifikasi live DB dulu → tidak ada celah write → tanpa migration baru |
| BUG-089 | **GET bebas data operasional** — `materials` (cost/stok), `suppliers` (PII kontak), `purchase-requests` (cost), `purchase-orders` (pembelian) bisa dibaca semua role; plus `purchase-orders` POST/PUT & `po-delivery` POST tidak cek `status='active'` (user nonaktif masih bisa aksi) | ✅ Fixed (Phase 1, 2026-08-13) | GET dibatasi role: materials/suppliers/PR → gudang/admin/owner; PO → +finance. Tambah cek `status='active'` di POST/PUT yang belum. Alasan: data biaya = sensitif; user nonaktif tak boleh aksi (pola fail-closed) |
| BUG-090 | **Client fail-open `role ?? 'admin'`** — login, dashboard layout, survey detail menganggap user tanpa profil = admin (UI tampil admin walaupun server menolak) | ✅ Fixed (Phase 1, 2026-08-13) | Fail-closed: role null → signout/redirect login (login & layout), role `''` di survey (UI tak tampil tombol edit/delete). Alasan: konsisten deny-by-default (F-21); server API sudah menolak, UI harus selaras |
| BUG-091 | **Rate limit hanya di 1/33 route** (`setup-accounts`) — `upload` (storage DoS), `create-staff` (brute-force akun), SEO upload, dan 5 route TikTok sync tanpa throttle (memanggil API eksternal + insert jurnal) | ✅ Fixed (Phase 2, 2026-08-13) | Terapkan `checkRateLimit` (in-memory, IP) ke `upload` (60/menit), `create-staff`, `seo/upload-*`, `tiktok/auth` POST/PUT, `tiktok/auth/reauthorize`, `tiktok/sync-orders`, `sync-finance`, `sync-to-main-orders`, `create-piutang`. Alasan: helper sudah ada di `lib/auth.ts`; mencegah DoS & abuse biaya eksternal |
| BUG-092 | **`create-staff` lemah** — requester tak dicek `status='active'` (admin dinonaktifkan masih bisa buat akun); password min 6 (lemah utk akun backend); error auth (mis. "email sudah terdaftar") bocor → email enumeration; role `laundry` absen dari enum API & UI (inkonsisten dgn 8 role) | ✅ Fixed (Phase 2, 2026-08-13) | Tambah cek `status='active'`; password min **8** (UI `minLength={8}` sudah sesuai); error auth di-redaksi (detail hanya log server); tambah role `laundry` di enum API + `ROLES`/`ROLE_COLORS` UI. Alasan: akun = surface serangan; konsistensi 8 role (SOP #4) |
| BUG-093 | **TikTok OAuth `state` = shop id (predictable)** — attacker yang tahu shop_id bisa memanipulasi callback; state bukan random nonce | ✅ Fixed (Phase 2, 2026-08-13, migration 084) | `state` = `crypto.randomBytes(24)` nonce, disimpan di kolom `tiktok_shop_settings.oauth_state`; callback mencocokkan via nonce & **menghapus setelah dipakai** (single-use, anti-replay); `reauthorize` ikut pola sama. Alasan: `code` OAuth memang single-use, tapi state random = defense-in-depth CSRF |
| BUG-094 | **Asimetri rollback jurnal finansial** — `handlePay` (payments) rollback penuh (BUG-073), TAPI refund/hutang/piutang/payroll/aset hanya toast warning saat jurnal gagal → transaksi tercatat tanpa jurnal → **ledger divergen dari data operasional** | ✅ Fixed (Phase 3, 2026-08-13) | **Seragamkan pola BUG-073 di SEMUA jalur**: jurnal gagal → rollback penuh (hapus row baru / kembalikan paid_amount & status semula). Diterapkan di: `payments.handleRefund`, `hutang.handlePayment`, `piutang/faktur` (create + pay + adjust), `piutang/process`, `laundry-payroll.markAsPaid`, `assets.create`. Alasan (SOP #5): uang wajib idempotent + rollback penuh, jangan biarkan ledger bocor |
| BUG-095 | **Hardcoded UUID akun di client + double-count saldo** — UUID akun (Peralatan/Kas/Modal/Piutang/Penjualan) hardcoded di assets/settings/piutang (rentan drift saat DB reset); `accounts/accounts` menghitung saldo N+1 query + **menambah `accounts.balance`** (double-count) padahal laporan pakai `ledger.ts` yang mengabaikan balance → angka beda antar halaman | ✅ Fixed (Phase 3, 2026-08-13) | (1) Helper `getAccountIdByCode(supabase, code)` di `config/accounts.ts` — lookup by **code stabil** (bukan UUID), dipakai di assets/settings/piutang process/faktur. (2) `accounts/accounts` → pakai `fetchAccountBalances` (satu sumber kebenaran sama dgn laporan), field "Saldo Awal" dihapus dari form COA (saldo awal diatur via Finance → Settings). Alasan (SOP #4): satu sumber kebenaran, anti-drift |
| BUG-096 | **PO paid tanpa jurnal di `owner/suppliers`** — judul tombol "Bayar" mengklaim "jurnal hutang dibuat otomatis" tapi kode hanya update status PO (tanpa `hutang_paid`) → liabilitas & ledger bocor; duplikasi jalur dgn API `purchase-orders/[id]` yang sudah bikin jurnal | ✅ Fixed (Phase 3, 2026-08-13) | `updatePOStatus('paid')` kini buat jurnal `hutang_paid` idempotent (`po_paid:<id>`) + rollback (PO tidak ditandai paid jika jurnal gagal) — seragam dgn jalur API. Alasan (SOP #4): hapus jalur divergen, satu perilaku di UI & API |
| BUG-097 | **`markAsPaid` payroll tanpa idempotency_key** — payroll di-reopen lalu di-mark paid lagi → jurnal `expense_paid` dobel | ✅ Fixed (Phase 3, 2026-08-13) | Tambah `idempotency_key: laundry_payroll_paid:<id>` + rollback penuh (jurnal gagal → payroll kembali ke pending). Alasan (SOP #5): anti double-journal |
| BUG-098 | **`kronologi-hpp` misnamed + `.limit(200)`** — laporan bernama "Kronologi HPP" tapi isinya kronologi OMZET (bukan harga pokok penjualan) → nama menipu; `.limit(200)` → periode lama selalu terpotong | ✅ Fixed (Phase 4, 2026-08-13) | **Rename ke "Kronologi Omzet"** (file `kronologi-omzet.tsx`, route finance & owner, label hub, PDF filename) + **pagination server-side** (`range` + `count exact`, default 50). Alasan: nama harus jujur dgn isi; laporan akurat utk data besar |
| BUG-099 | **`lte '...-31'` di owner/marketplace** — bulan 30 hari (Apr/Jun/Sep/Nov) & Februari order-nya tidak pernah terhitung (query sampai tanggal 31 yang tidak ada) | ✅ Fixed (Phase 4, 2026-08-13) | Hitung akhir bulan dinamis via `new Date(year, month, 0).getDate()` + batas waktu T00:00/T23:59. Alasan: konsisten dgn fix timezone BUG-075 |
| BUG-100 | **Rumus sisa piutang berpotensi divergen** — meski F-61 sudah seragam (kurangi fee), tidak ada satu sumber kebenaran → drift bisa muncul lagi | ✅ Fixed (Phase 4, 2026-08-13) | Helper `piutangSisa()` di `lib/ledger.ts` (amount − paid − return − fee, clamped ≥ 0); dipakai di `umur-piutang` & finance dashboard. Alasan (SOP #4): satu sumber kebenaran, cegah regresi berulang |
| BUG-101 | **`admin/reports` `.limit(200)` + filter client** — laporan sales periode lama terpotong 200 order; MoM dihitung dari data terpotong | ✅ Fixed (Phase 4, 2026-08-13) | Filter periode pindah ke SERVER (`gte/lte` current + prev utk MoM), tanpa limit. Alasan: akurasi laporan utk semua data |
| BUG-102 | **`admin/portfolio` (`.limit(50)`) & `admin/laundry` (`.limit(100)`) tanpa pagination** — post/order laundry lama tak pernah tampil saat data bertambah | ✅ Fixed (Phase 5, 2026-08-13) | Portfolio: pagination server-side (range + count, 12/halaman). Laundry: pagination client-side pada `filtered` (20/halaman), reset halaman saat search/filter berubah. Alasan: konsisten dgn pola Pagination di halaman lain |
| BUG-103 | **`theme_preset` tidak berubah ke `custom` saat edit warna manual** — user pilih preset lalu ubah satu warna, kartu preset tetap menandai preset lama yang warnanya sudah tak cocok | ✅ Fixed (Phase 5, 2026-08-13) | `updateThemeColor()` — set warna + preset ke `custom` bila hasil edit tidak lagi cocok dgn preset aktif. Alasan: UI jujur thd kondisi sebenarnya |
| BUG-104 | **`handleSave` landing tak deteksi "0 rows updated"** — row `landing_settings (key=hero)` hilang → update sukses 0 baris → toast "Settings saved successfully!" palsu | ✅ Fixed (Phase 5, 2026-08-13) | Cek `count` hasil update → `count === 0` = toast error. Alasan: anti feedback palsu |
| BUG-105 | **Kredensial default hardcoded di `setup/page.tsx`** — `owner@kj.com/kjowner123` & `admin@kj.com/kjadmin123` tampil di client bundle → bisa dipakai tanpa sadar | ✅ Fixed (Phase 5, 2026-08-13) | Default dihapus → field kosong (placeholder). Alasan: kredensial tidak boleh bocor di bundle |
| BUG-106 | **Karakter Cina korup di installer checklist** — `'Pasang ker HEADER/熟知'` (simbol Cina) & `'Rapikan& rapikan kiri-kanan'` (ampersand aneh) tampil ke installer | ✅ Fixed (Phase 5, 2026-08-13) | Ganti → `'Pasang kepala/header'` & `'Rapikan kiri-kanan'`. Catatan: karakter Cina di `src/lib/tiktok-shop-sdk/` adalah komentar file SDK auto-generated (dead code, tak tampil di UI) |
| BUG-107 | **Dead code `src/lib/tiktok-shop-sdk/` (1.971 file)** — SDK auto-generated TikTok Shop yang TIDAK pernah di-import aplikasi (integrasi aktual via `lib/tiktok.ts` + fetch manual); dependensi `request` & `@types/request` hanya dipakai SDK ini; membuat build lebih berat & noise | ✅ Fixed (Phase 6A, 2026-08-13) | Hapus permanen folder SDK (1.971 file). Proses anti-regresi: pindah ke `_dead/` → `tsc`+`build` hijau → hapus permanen → hapus `request`/`@types/request` dari package.json + npm install → `tsc`+`build`+`vitest` 24/24 hijau. Verifikasi `rg "tiktok-shop-sdk"` kosong |
| BUG-108 | **Nav laporan keuangan duplikat** — `finance/laporan/page.tsx` & `owner/laporan/page.tsx` copy-paste identik (beda href saja) → fix di satu halaman tidak ikut halaman lain | ✅ Fixed (Phase 6C, 2026-08-13) | Shared `components/reports/ReportsNav.tsx` (prop `basePath`), kedua route jadi wrapper tipis. Alasan (SOP #4): satu sumber kebenaran, hapus duplikasi |
| BUG-109 | **Notifikasi polling 30s (bukan realtime)** — `NotificationBell` pakai `setInterval(30s)`; notifikasi baru (mis. survey masuk) tertunda hingga 30 detik; tidak konsisten dgn pola `postgres_changes` di halaman lain | ✅ Fixed (Phase 6D, 2026-08-13, migration 085) | Aktifkan Realtime: `ALTER PUBLICATION supabase_realtime ADD TABLE notifications` (idempotent, guard pg_publication_tables). `NotificationBell` → `supabase.channel().on('postgres_changes', { event: 'INSERT', filter: user_id=eq.<id> })`, cleanup `removeChannel` di unmount. RLS `notifications_own` (user_id = auth.uid()) konsisten dgn filter. Alasan: realtime = pola utama app (admin home & booking); polling = inkonsisten |
| BUG-110 | **Monolit `admin/orders/[id]` (3.561 baris)** — `LOG_ACTION` map & `DEFAULT_CHECKLIST` masih inline di page (sulit di-test, drift risiko dgn constraint `order_logs_action_check`) | ✅ Fixed (Phase 6B-1, 2026-08-13) | Pindah ke `lib/order-detail.ts` (`getOrderLogAction` + `DEFAULT_CHECKLIST`) + unit test (27 total, +3). Behavior-preserving. Alasan (SOP #4): satu sumber kebenaran, logika murni bisa di-test |
| BUG-111 | **Monolit `admin/orders/[id]` — 5 modal besar inline** (Jadwalkan Pasang ~120 baris, Foto Progress ~160, Cancel, Return ~200, Payment ~110) → page sulit dipelihara & di-test | ✅ Fixed (Phase 6B-2, 2026-08-13) | Ekstrak ke `components/orders/`: `ScheduleInstallModal`, `PhotoUploadModal`, `CancelOrderModal`, `ReturnModal`, `PaymentModal` (state & handler tetap di parent — behavior-preserving). Page turun 3.561 → 2.923 baris (−638). Alasan (SOP #4): komponen modular, tiap modal bisa diuji sendiri |
| BUG-112 | **Monolit `admin/orders/[id]` — section render besar inline** (pipeline stepper ~110 baris, Hasil Survey + modal pilih ~90 baris, Estimasi+Customer+OrderInfo ~150 baris) | ✅ Fixed (Phase 6B-3, 2026-08-13) | Ekstrak `OrderPipelineStepper`, `OrderSurveySection`, `OrderSummarySection` (read-only, behavior-preserving). Page turun 2.923 → 2.611 baris (total −950 dari 3.561 asli). Alasan (SOP #4): section modular |
| BUG-113 | **Monolit `admin/orders/[id]` — OrderItems + AddItemModal (~950 baris) + Checklist inline** — form tambah item 3 tipe (gorden/perabot/laundry) + BOM hint + estimasi laundry adalah blok terbesar di page | ✅ Fixed (Phase 6B-3d, 2026-08-13) | Ekstrak `OrderItemsTable`, `PreparationChecklist`, `AddItemModal` (props lengkap state+handlers, behavior-preserving). Page turun 2.611 → **1.490 baris** (total −2.071 dari 3.561 asli). Verifikasi browser Playwright 10/10 (modal terbuka, form render, checklist toggle). Alasan (SOP #4): blok terbesar dipisah, tiap komponen bisa diuji |
| BUG-114 | **Monolit `admin/orders/[id]` — seluruh state & handlers inline di komponen** (~980 baris) → page sulit dipelihara & logika tidak reusable | ✅ Fixed (Phase 6B-4, 2026-08-13) | Semua state (~40) + handlers dipindah ke **`useOrderDetail(id)` hook** (`src/lib/use-order-detail.ts`); page jadi komposisi murni (**505 baris**, −85% dari 3.561 asli). Verifikasi browser Playwright 10/10 (pasang + kirim render, AddItemModal). Alasan (SOP #4): logika reusable & terpisah dari UI |
| BUG-115 | **RLS katalog publik terbuka (audit)** — `products`, `categories`, `banners`, `portfolio_posts`, `bom` punya policy write `FOR ALL authenticated` → semua staff login (penjahit/surveyor/installer/laundry) bisa buat/ubah/hapus produk & konten website (analog BUG-081); `is_finance_role()` & `rls_auto_enable()` bisa dieksekusi `anon` (melanggar SOP); `users` SELECT semua authenticated | ✅ Fixed (audit, migration 087, 2026-08-13) | (1) Katalog+bom: write → `is_admin_or_owner_sd()` (SELECT publik tetap). (2) `REVOKE anon` + `REVOKE PUBLIC` utk `is_finance_role` & `rls_auto_enable` (GRANT authenticated). (3) `users` SELECT → `is_staff_active_sd()`. **Verifikasi user-level**: penjahit INSERT products → ditolak 42501; admin → sukses. Juga cleanup duplikat `cash_accounts` (19 baris Kas → 1), drop 7 index tak terpakai, tambah 15 index FK hot, `order_totals` → security_invoker, `SET search_path` 3 fungsi |

### Dead code — status aktual (2026-08-13)
- **Route API DIHAPUS** (086/sesi 34, keputusan user — tidak ada caller produksi, UI berjalan via Supabase client langsung): `api/customers`, `api/landing-settings`, `api/materials`, `api/products`, `api/suppliers`, `api/purchase-orders` (+`[id]`), `api/purchase-requests` (+`[id]`), `api/install-bookings` (base). **Dipertahankan** (dipakai): `api/orders` base (smoke test), `api/orders/[id]`, `api/install-bookings/[id]`, `api/notifications`, `api/surveys`, `api/journal`, `api/upload`, `api/seo/*`, `api/tiktok/*`, `api/setup-accounts`, `api/admin/create-staff`, `api/gudang/po-delivery`, `api/webhooks/*`.
- **Tabel DI-DROP** (086, keputusan user — 0 referensi kode): `packing_checklists`, `return_requests`, `order_preparation_checklist` (singular). **Dipertahankan** (ditulis RPC produksi aktif `consume_materials_for_production`): `low_stock_alerts`, `order_material_consumption`. `seo_settings` di-drop migration 079.
- **RPC DI-DROP** (086, keputusan user — 0 referensi): `decrement_stock_gudang`, `get_material_stock`, `get_product_stock`, `update_cash_account_balance`. **Dipertahankan**: `rls_auto_enable` (dipanggil event trigger `ensure_rls` utk RLS otomatis).
- **Export DIHAPUS**: `clientError` (`src/lib/api-errors.ts:14`) — nol referensi.
