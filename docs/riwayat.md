...
> Satu dokumen konsolidasi: **riwayat perbaikan per fase** + **tracker bug lengkap (BUG-001 s/d BUG-135)** + **audit modul finance** + **backlog**.
...
## 2. Status Bug — Tabel Lengkap (BUG-001 s/d BUG-135)
...
|---|---|---|---|
| 1 | **E2E suite (chromium)** | 🟠 High | ✅ `tests/e2e/` — **38/38 pass** (2026-08-15): login 8 role, security (penjahit redirect + API 403), pipeline kirim 9 tahap, pasang 10 tahap, finance, katalog/BOM, mobile-charts (sesi 50), dsb. Jalankan: `npx playwright test --project=chromium` (butuh dev server / auth setup live). ⚠️ Sesudah sesi 59 (bukti foto wajib): flow create-order E2E perlu update — setiap input DP/payment harus upload bukti foto dulu |
