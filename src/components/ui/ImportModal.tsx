...
interface ImportModalProps {
  ...
  onImport: (rows: Record<string, string | number | null>[]) => Promise<{ inserted: number; updated: number; errors: string[] }>
  entityName?: string
}
...
export default function ImportModal({
  ...
  const allMapped = columns.filter(c => c.required).every(c => mapping.get(c.key) !== null && mapping.get(c.key) !== undefined)
  ...
  return (
    ...
    >
      <div style={{ background: '#fff', borderRadius: '0.875rem', width: '100%', maxWidth: 800, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
        {/* Header */}
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '1.5rem' }}>
          {/* Success Result */}
          {result && (
            <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '1.5rem' }}>
                ...
                {result.errors.length > 0 && (
                  <details style={{ marginTop: '0.5rem' }}>
                    ...
                    {result.errors.slice(0, 10).map((e, i) => (
                      <div key={i} style={{ marginLeft: '1rem', fontSize: '0.8rem', color: '#dc2626' }}>• {e}</div>
                    ))}
                    {result.errors.length > 10 && <div style={{ marginLeft: '1rem', fontSize: '0.8rem' }}>...and {result.errors.length - 10} more</div>}
                  </details>
                )}
              </div>
              <button onClick={onClose} style={{ marginTop: '1rem', padding: '0.5rem 1rem', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', fontWeight: 600 }}>
                Tutup
              </button>
            </div>
          )}
          ...
          {!file && (
            <div
              ...
              style={{ border: '2px dashed #d1d5db', borderRadius: '0.75rem', padding: '3rem', textAlign: 'center', cursor: 'pointer' }}
            >
          ...
          {file && csvHeaders.length > 0 && !result && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                ...
                <span style={{ fontSize: '0.875rem', color: '#166534' }}>File: <strong>{file.name}</strong> ({csvRows.length} baris)</span>
                <button onClick={() => { setFile(null); setCsvHeaders([]); setCsvRows([]); setMapping(new Map()); setParsed([]) }}
                  style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: '#dc2626' }}>
                  Ganti file
                </button>
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '0.75rem' }}>
                  {columns.map(col => (
                    <div key={col.key} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '0.75rem', fontWeight: '600', color: col.required ? '#cc7030' : '#6b7280' }}>
                      ...
                      <select
                        ...
                        style={{ flex: 1, padding: '0.375rem', border: '1px solid #d1d5db', borderRadius: '0.375rem', fontSize: '0.8rem' }}
                      >
              ...
              {parsed.length > 0 && (
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.5rem' }}>Preview (5 baris pertama)</div>
                  <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: '0.5rem' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem' }}>
                      <thead>
                        <tr>
                          {columns.filter(c => mapping.get(c.key) !== null).map(c => (
                            <th key={c.key} style={{ padding: '0.5rem 0.75rem', background: '#f9fafb', borderBottom: '1px solid #e5e7eb', textAlign: 'left', whiteSpace: 'nowrap' }}>{c.label}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {parsed.map((row, i) => (
                          <tr key={i}>
                            {columns.filter(c => mapping.get(c.key) !== null).map(c => (
                              <td key={c.key} style={{ padding: '0.375rem 0.75rem', borderBottom: '1px solid #f3f4f6' }}>
                                {String(row[c.key] ?? '—')}
                              </td>
                            ))}
                          </tr>
