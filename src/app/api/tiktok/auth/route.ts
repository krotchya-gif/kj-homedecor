...
export async function GET(req: NextRequest) {
  ...
  if (code && state) {
    ...
    if (!settings) {
      return NextResponse.redirect(new URL('/owner/tiktok?error=settings_not_found', req.url))
    }
    ...
    try {
      ...
      const res = await fetch(`https://auth.tiktok-shops.com/api/v2/token/get?${qs}`)
      ...
      return NextResponse.redirect(new URL('/owner/tiktok?success=connected', req.url))
    } catch (err: any) {
      return NextResponse.redirect(new URL(`/owner/tiktok?error=${encodeURIComponent(err.message)}`, req.url))
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!app_key || !app_secret) {
    return NextResponse.json({ error: 'app_key and app_secret are required' }, { status: 400 })
  }

  const { data, error } = await supabase.from('tiktok_shop_settings').insert({
