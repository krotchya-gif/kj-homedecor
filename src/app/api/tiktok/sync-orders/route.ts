...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const orderListRes = await fetch('https://open-api.tiktokglobalshop.com/order/202309/orders/search', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        page_size: 100,
        ...(start_date ? { create_time_ge: Math.floor(new Date(start_date).getTime() / 1000) } : {}),
        ...(end_date ? { create_time_lt: Math.floor(new Date(end_date).getTime() / 1000) } : {}),
      }),
    })
