...
export default function FinanceSettingsPage() {
	...
	async function fetchData() {
		...
		const hutTotal = (hutangRes.data ?? []).reduce(
			(s: number, h: any) =>
				s + (h.amount ?? 0) - (h.paid_amount ?? 0) - (h.return_amount ?? 0),
			0,
		);
		...
		const piuTotal = (piutangRes.data ?? []).reduce(
			(s: number, p: any) =>
				s + (p.amount ?? 0) - (p.paid_amount ?? 0) - (p.return_amount ?? 0),
			0,
		);
		...
		<div>
			<div className="page-header">
				...
				<p className="page-subtitle">
					Input saldo awal dan pengaturan akun keuangan
				</p>
			</div>
				...
				<>
					{activeTab === "saldo" && (
						<div>
							...
							>
								Atur saldo awal untuk setiap akun kas, bank, dan e-wallet.
								Gunakan fitur ini saat pertama kali menggunakan sistem keuangan.
							</p>
					...
					{activeTab === "hutang" && (
						<div>
							...
							>
								Ringkasan hutang supplier saat ini. Data ini berasal dari
								tagihan yang sudah dicatat di menu Hutang.
							</p>
							<div
								style={{
									display: "grid",
							...
							<p style={{ fontSize: "0.8rem", color: "#9ca3af" }}>
								Untuk input saldo awal hutang, gunakan menu{" "}
								<strong>Hutang → Tambah Tagihan</strong>. Saldo awal otomatis
								tercatat dari tagihan yang sudah ada.
							</p>
						</div>
					)}
					...
					{activeTab === "piutang" && (
						<div>
							...
							>
								Ringkasan piutang pelanggan saat ini. Data ini berasal dari
								faktur yang sudah dicatat di menu Piutang.
							</p>
							<div
								style={{
									display: "grid",
							...
							<p style={{ fontSize: "0.8rem", color: "#9ca3af" }}>
								Untuk input saldo awal piutang, gunakan menu{" "}
								<strong>Piutang → Faktur → Tambah Faktur</strong>. Saldo awal
								otomatis tercatat dari faktur yang sudah ada.
							</p>
						</div>
					)}
				</>
