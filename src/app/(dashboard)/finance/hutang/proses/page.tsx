...
export default function ProsesReturPage() {
	...
	function openReturForm(h: any) {
		setReturItem(h);
		const sisa =
			(h.amount ?? 0) - (h.paid_amount ?? 0) - (h.return_amount ?? 0);
		setReturForm({
			amount: String(sisa),
			reason: "",
	...
	async function handleRetur(e: React.FormEvent) {
		...
		const sisa =
			(returItem.amount ?? 0) - (returItem.paid_amount ?? 0) - newReturnAmount;
		...
		if (!error) {
			...
			const accountRes = await supabase
				...
				.eq("code", "5-1000")
				.single();
			const cashAccountRes = await supabase
				.from("cash_accounts")
				.select("account_id")
				.limit(1)
				.single();
						...
						<tbody>
							{filtered.map((h) => {
								const sisa =
									...
									(h.return_amount ?? 0);
								const bisaRetur =
									sisa > 0 && h.status !== "paid" && h.status !== "cancelled";
								return (
									<tr key={h.id}>
										...
										<td style={{ color: "#16a34a", textAlign: "right" }}>
											{formatRp(h.paid_amount ?? 0)}
										</td>
										<td style={{ color: "#dc2626", textAlign: "right" }}>
											{formatRp(h.return_amount ?? 0)}
										</td>
										<td
											style={{
												fontWeight: "700",
						...
						>
							...
							<div style={{ color: "#6b7280" }}>
								...
								<strong style={{ color: "#cc7030" }}>
									{formatRp(
										(returItem.amount ?? 0) -
											(returItem.paid_amount ?? 0) -
											(returItem.return_amount ?? 0),
									)}
								</strong>
							</div>
						</div>
						...
						>
							<div>
								...
								<input
									...
									onChange={(e) =>
										setReturForm((f) => ({ ...f, amount: e.target.value }))
									}
									style={{
										width: "100%",
										padding: "0.625rem",
							...
							<div>
								...
								<input
									...
									onChange={(e) =>
										setReturForm((f) => ({ ...f, date: e.target.value }))
									}
									style={{
										width: "100%",
										padding: "0.625rem",
							...
							<div>
								...
								<textarea
									...
									onChange={(e) =>
										setReturForm((f) => ({ ...f, reason: e.target.value }))
									}
									rows={2}
