...
export default function ProsesReturPage() {
  ...
  const [returForm, setReturForm] = useState({ amount: '', reason: '', date: '' })
  ...
  function openReturForm(h: any) {
    ...
    setReturForm({ amount: String(sisa), reason: '', date: new Date().toISOString().split('T')[0] })
    setShowRetur(true)
  }
      ...
      if (accountRes.data && cashAccountRes.data) {
        await fetch('/api/journal', {
          ...
          body: JSON.stringify({
            ...
            lines: [
              { account_id: cashAccountRes.data.account_id, debit: returAmount, credit: 0 },
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: '1.25rem', maxWidth: 320 }}>
        ...
        <input type="text" placeholder="Cari supplier atau invoice..." value={search} onChange={(e) => setSearch(e.target.value)}
              ...
              {filtered.map((h) => {
                ...
                return (
                  <tr key={h.id}>
                    ...
                    <td>
                      {bisaRetur ? (
                        <button onClick={() => openReturForm(h)}
                          style={{ padding: '0.25rem 0.625rem', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '0.375rem', fontSize: '0.72rem', fontWeight: '600', cursor: 'pointer' }}>
                          Retur
                        </button>
                      ) : (
      ...
      {showRetur && returItem && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          onClick={(e) => { if (e.target === e.currentTarget) setShowRetur(false) }}>
          <div style={{ background: '#fff', borderRadius: '0.875rem', padding: '2rem', width: '100%', maxWidth: 420, boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              ...
              <button onClick={() => setShowRetur(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280' }}><X size={20} /></button>
            </div>
            <div style={{ background: '#f9fafb', borderRadius: '0.5rem', padding: '0.75rem', marginBottom: '1rem', fontSize: '0.85rem' }}>
              <div style={{ color: '#6b7280', marginBottom: '0.25rem' }}>Supplier: <strong>{returItem.supplier?.name ?? '—'}</strong></div>
              <div style={{ color: '#6b7280', marginBottom: '0.25rem' }}>Invoice: <strong>{returItem.invoice_number ?? '—'}</strong></div>
              <div style={{ color: '#6b7280' }}>Sisa Tagihan: <strong style={{ color: '#cc7030' }}>{formatRp((returItem.amount ?? 0) - (returItem.paid_amount ?? 0) - (returItem.return_amount ?? 0))}</strong></div>
            </div>
            <form onSubmit={handleRetur} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Jumlah Retur (Rp) *</label>
                <input type="number" required placeholder="0" value={returForm.amount} onChange={(e) => setReturForm(f => ({ ...f, amount: e.target.value }))}
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Tanggal Retur</label>
                <input type="date" value={returForm.date} onChange={(e) => setReturForm(f => ({ ...f, date: e.target.value }))}
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Alasan Retur</label>
                <textarea value={returForm.reason} onChange={(e) => setReturForm(f => ({ ...f, reason: e.target.value }))} rows={2} placeholder="Misal: barang cacat, salah kirim..."
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                <button type="button" onClick={() => setShowRetur(false)} style={{ flex: 1, padding: '0.75rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', background: '#fff', cursor: 'pointer', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem' }}>
                  <X size={16} /> Batal
                </button>
                <button type="submit" disabled={saving} style={{ flex: 1, padding: '0.75rem', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.375rem' }}>
