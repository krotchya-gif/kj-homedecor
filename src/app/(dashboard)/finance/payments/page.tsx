...
export default function FinancePaymentsPage() {
	...
	const [activeTab, setActiveTab] = useState<"payment" | "refund" | "qc">(
		"payment",
	);
	...
	async function load() {
		...
		const qcApproved = (qcData.data ?? []).filter(
			(sq: any) => sq.order?.payment_status !== "paid",
		);
	...
	async function handlePay(e: React.FormEvent) {
		...
		const newDp =
			payForm.type === "dp" ? selected.dp_amount + amount : selected.dp_amount;
		const newLunas =
			payForm.type === "lunas"
				? selected.lunas_amount + amount
				: selected.lunas_amount;
	...
	async function handleApprove(order: any) {
		...
		const { data: freshOrder, error: fetchErr } = await supabase
			.from("orders")
			.select(
				"id, status, dp_amount, lunas_amount, total_amount, payment_status",
			)
			.eq("id", order.id)
			.single();
		...
		if (paidSum < freshOrder.total_amount) {
			toast(
				"error",
				`Gagal Approve — Sisa pembayaran ${fmt(freshOrder.total_amount - paidSum)}, order belum lunas!`,
			);
			return;
		}
		if (freshOrder.payment_status !== "paid") {
			toast(
				"error",
				`Gagal Approve — payment_status belum 'paid' (saat ini: ${freshOrder.payment_status}).`,
			);
			return;
		}
		...
		if (freshOrder.status === "ready") {
			...
			toast(
				"info",
				"Order sudah di Dikemas. Gudang tinggal input resi (di /admin/shipping).",
			);
			load();
			return;
		} else if (
	...
	async function handleQcApprove(order: any) {
		...
		if (paidSum < (order.total_amount ?? 0)) {
			toast(
				"error",
				`Gagal Approve — Sisa pembayaran ${fmt((order.total_amount ?? 0) - paidSum)}, order belum lunas!`,
			);
			return;
		}
		...
		toast(
			"success",
			'Order berhasil diapprove! Status berubah menjadi "Siap Kirim". Order siap dikemas dan dikirim.',
		);
		load();
	}
	...
	async function handleRefund(returnRecord: any) {
		...
		toast(
			"success",
			`Refund ${fmt(returnRecord.refund_amount)} berhasil diproses!`,
		);
		load();
	}
		...
		<div>
			<div className="page-header">
				...
				<p className="page-subtitle">
					DP/Lunas tracking — Payment Gate aktif sebelum order bisa dikirim
				</p>
			</div>
			...
			>
				...
				<div style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.85)" }}>
					<strong style={{ color: "#f4a857" }}>Payment Gate aktif:</strong>{" "}
					Order hanya bisa lanjut ke <strong>packing</strong> jika{" "}
					<strong style={{ color: "#fff" }}>(1)</strong> payment_status = 'paid'
					(DP + Lunas ≥ Total), dan{" "}
					<strong style={{ color: "#fff" }}>(2)</strong> ada record pembayaran
					dengan verified_by. Finance harus approve manual dari status{" "}
					<strong>Siap</strong> ke <strong>Cek Bayar</strong>.
				</div>
			</div>
						...
						val: fmt(
							Math.max(
								...
								orders
									...
									.reduce(
										(s, o) =>
											s + (o.total_amount - o.dp_amount - o.lunas_amount),
										0,
									),
							),
						),
			...
			>
				{[
					...
					{ key: "qc", label: "✅ QC Approved", count: qcOrders.length },
					{
						key: "refund",
						label: "💸 Refund",
						count: refundList.filter((r) => r.refund_status === "pending")
							.length,
					},
				].map((t) => (
					<button
				...
				<div style={{ marginBottom: "1.5rem" }}>
					...
					>
						<div style={{ fontSize: "0.85rem", color: "#92400e" }}>
							<strong>💸 Refund Tab</strong> — Proses refund untuk order yang
							di-return customer.
						</div>
					</div>
					{refundList.filter((r) => r.refund_status === "pending").length ===
					0 ? (
						<div
							style={{
								padding: "3rem",
									...
									{refundList
										...
										.map((r) => (
											<tr key={r.id}>
												...
												<td style={{ fontWeight: "700", color: "#cc7030" }}>
													{fmt(r.refund_amount)}
												</td>
												<td>
													<span
							...
							<table>
								...
								<tbody>
									{filtered.map((o) => {
										...
										const canApprove =
											o.payment_status === "paid" && o.status === "ready";
										return (
											<tr key={o.id}>
												<td style={{ fontWeight: "500" }}>
													{o.customer?.name ?? "—"}
					...
					{!loading && filtered.length > 0 && (
						...
						>
							<span style={{ fontSize: "0.8rem", color: "#6b7280" }}>
								...
								{Math.max(1, Math.ceil(totalCount / PAGE_SIZE))} — {totalCount}{" "}
								order
							</span>
				...
				<div>
					...
					>
						<div style={{ fontSize: "0.85rem", color: "#166534" }}>
							<strong>✅ QC Approved</strong> — Order yang sudah LULUS QC
							Steam/QC dari Gudang.
						</div>
					</div>
					{qcOrders.length === 0 ? (
						<div
						...
						>
							<CheckCircle2
								size={32}
								style={{ opacity: 0.3, margin: "0 auto 0.75rem" }}
							/>
											...
											<tr key={qc.id}>
												...
												<td>
													<span
														style={{
															background:
																PAYMENT_COLORS[o?.payment_status ?? "pending"]
																	.bg,
															color:
																PAYMENT_COLORS[o?.payment_status ?? "pending"]
																	.text,
					...
					>
						...
						>
							...
							>
								...
								/>
								Pembayaran ini akan langsung diverifikasi oleh{" "}
								<strong>{currentUser?.name ?? "Finance"}</strong>
							</div>
							<div style={{ display: "flex", gap: "0.75rem" }}>
								<button
									type="button"
