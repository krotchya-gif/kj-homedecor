...
export default function TransferPage() {
  ...
  const [form, setForm] = useState({
    from_account_id: '', to_account_id: '', amount: '', description: '', entry_date: '',
  })
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!json.error) {
      ...
      await supabase.from('cash_accounts').update({ balance: fromAcc.balance - amount }).eq('id', form.from_account_id)
      await supabase.from('cash_accounts').update({ balance: toAcc.balance + amount }).eq('id', form.to_account_id)
    }
    setSaving(false)
    setShowForm(false)
    fetchData()
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.625rem 1.25rem', background: '#6366f1', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: '600', fontSize: '0.875rem', cursor: 'pointer' }}>
              ...
              {filtered.map((j: any) => {
                ...
                return (
                  <tr key={j.id}>
                    ...
                    <td style={{ color: '#dc2626', fontSize: '0.82rem' }}>{creditLine?.account_id?.substring(0, 8) ?? '—'}</td>
                    <td style={{ color: '#16a34a', fontSize: '0.82rem' }}>{debitLine?.account_id?.substring(0, 8) ?? '—'}</td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          ...
          <div style={{ background: '#fff', borderRadius: '0.875rem', padding: '2rem', width: '100%', maxWidth: 480, boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Dari Akun *</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Ke Akun *</label>
                <select required value={form.to_account_id} onChange={(e) => setForm(f => ({ ...f, to_account_id: e.target.value }))}
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none', background: '#fff' }}>
                  <option value="">— Pilih Akun Tujuan —</option>
                  {cashAccounts.filter(c => c.id !== form.from_account_id).map(c => <option key={c.id} value={c.id}>{c.bank_name} — {c.account_number} ({formatRp(c.balance)})</option>)}
                </select>
                {form.from_account_id && form.from_account_id === form.to_account_id && (
                  <p style={{ color: '#dc2626', fontSize: '0.78rem', marginTop: '0.25rem' }}>Akun asal dan tujuan harus berbeda</p>
                )}
              </div>
              ...
              <div>
                ...
                <input type="text" value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Misal: Setor tunai ke bank"
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none' }} />
              </div>
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                ...
                <button type="submit" disabled={saving || form.from_account_id === form.to_account_id} style={{ flex: 1, padding: '0.75rem', background: '#6366f1', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: '600' }}>
