...
export default function ExpensePage() {
  ...
  const [form, setForm] = useState({
    cash_account_id: '', expense_account_id: '', amount: '', description: '', entry_date: '',
  })
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.625rem 1.25rem', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: '600', fontSize: '0.875rem', cursor: 'pointer' }}>
      ...
      <div className="data-table">
        {loading ? (
          ...
          <table>
            ...
            <tbody>
              {filtered.map((j: any) => {
                const expenseLine = j.lines?.find((l: any) => l.debit > 0)
                ...
                return (
                  <tr key={j.id}>
                    ...
                    <td style={{ color: '#6b7280', fontSize: '0.82rem' }}>{expenseLine?.account_id?.substring(0, 8) ?? '—'}</td>
                    <td style={{ color: '#6b7280', fontSize: '0.82rem' }}>{cashLine?.account_id?.substring(0, 8) ?? '—'}</td>
                    <td style={{ fontWeight: '600', color: '#dc2626', textAlign: 'right' }}>{formatRp(cashLine?.credit ?? 0)}</td>
                  </tr>
                )
              })}
            </tbody>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          ...
          <div style={{ background: '#fff', borderRadius: '0.875rem', padding: '2rem', width: '100%', maxWidth: 480, boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1.5rem' }}>Tambah Pengeluaran</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Akun Beban *</label>
                <select required value={form.expense_account_id} onChange={(e) => setForm(f => ({ ...f, expense_account_id: e.target.value }))}
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Sumber Dana (Kas/Bank) *</label>
              ...
              <div>
                ...
                <input type="text" required value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Misal: Listrik, ATK, Transport..."
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none' }} />
              </div>
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                ...
                <button type="submit" disabled={saving} style={{ flex: 1, padding: '0.75rem', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: '600' }}>
