...
export default function CashMutationPage() {
  ...
  useEffect(() => {
    supabase.from('cash_accounts').select('*, account:accounts(name)').eq('is_active', true).then(({ data }) => {
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)}
          style={{ padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none', background: '#fff', minWidth: 250 }}>
        ...
        <div style={{ display: 'flex', gap: '0.375rem' }}>
          {[
            ...
            <button key={opt.value} onClick={() => setDateRange(opt.value)}
              style={{ padding: '0.5rem 0.875rem', border: `1px solid ${dateRange === opt.value ? '#cc7030' : '#d1d5db'}`, borderRadius: '0.375rem', background: dateRange === opt.value ? '#cc7030' : '#fff', color: dateRange === opt.value ? '#fff' : '#374151', cursor: 'pointer', fontWeight: '600', fontSize: '0.78rem' }}>
              {opt.label}
            </button>
          ))}
        </div>
      ...
      {cashAcc && (
        <div style={{ background: 'linear-gradient(135deg, #1f2937, #374151)', borderRadius: '0.75rem', padding: '1.25rem', marginBottom: '1.25rem', color: '#fff' }}>
          <div style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#9ca3af', marginBottom: '0.25rem' }}>{cashAcc.bank_name}</div>
          ...
          <table>
            ...
            <tbody>
              {runningBalance.map((j: any, idx) => (
                <tr key={j.id ?? idx}>
                  <td style={{ color: '#6b7280' }}>{j.entry?.entry_date ? new Date(j.entry.entry_date).toLocaleDateString('id-ID') : '—'}</td>
                  <td style={{ fontWeight: '500' }}>{j.entry?.description ?? '—'}</td>
                  <td style={{ fontSize: '0.72rem', color: '#9ca3af' }}>{j.entry?.reference_type ?? '—'}</td>
                  <td style={{ fontWeight: '600', color: '#16a34a', textAlign: 'right' }}>{Number(j.debit) > 0 ? formatRp(Number(j.debit)) : '—'}</td>
                  <td style={{ fontWeight: '600', color: '#dc2626', textAlign: 'right' }}>{Number(j.credit) > 0 ? formatRp(Number(j.credit)) : '—'}</td>
