...
export default function CashMutationPage() {
	...
	useEffect(() => {
		...
		supabase
			.from("journal_lines")
			.select(
				"*, entry:journal_entries!inner(entry_date, description, reference_type)",
			)
		...
		<div>
			<div className="page-header">
				...
				<p className="page-subtitle">
					Riwayat transaksi per akun kas/bank/e-wallet
				</p>
			</div>
			...
			{cashAcc && (
				...
				>
					...
					<div
						style={{ fontSize: "1.5rem", fontWeight: "700", color: "#34d399" }}
					>
						{formatRp(cashAcc.balance)}
					</div>
				</div>
			)}
							...
							{runningBalance.map((j: any, idx) => (
								<tr key={j.id ?? idx}>
									<td style={{ color: "#6b7280" }}>
										{j.entry?.entry_date
											? new Date(j.entry.entry_date).toLocaleDateString("id-ID")
											: "—"}
									</td>
