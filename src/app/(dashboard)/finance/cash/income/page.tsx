...
export default function IncomePage() {
  ...
  const [revenueAccounts, setRevenueAccounts] = useState<{ id: string; name?: string; code?: string }[]>([])
          ...
          <MobileCards items={filtered} keyOf={(j) => j.id} renderCard={(j) => (
            <div className="mobile-card">
                ...
                <div className="mobile-card-row">
                  ...
                  <span className="mobile-card-value">{j.credit ?? j.amount}</span>
                </div>
            </div>
          )} />
        )}
          ...
          <table>
            ...
            <tbody>
              {filtered.map((j: JournalEntry) => {
                const cashLine = j.lines?.find((l: JournalLine) => l.debit > 0)
                const revLine = j.lines?.find((l: JournalLine) => l.credit > 0)
                return (
                  <tr key={j.id}>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Misal: Jual aset, refund supplier..."
              style={{
                width: '100%',
                padding: '0.625rem',
                border: '1px solid #d1d5db',
