...
export default function AccountsListPage() {
  ...
  const [form, setForm] = useState({
    code: '', name: '', type: 'asset' as typeof ACCOUNT_TYPES[number],
    balance: '', is_cash_account: false, description: '',
  })
  ...
  function openEdit(a: Account) {
    ...
    setForm({
      ...
      balance: String(a.balance ?? 0), is_cash_account: a.is_cash_account ?? false,
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const payload = {
      ...
      balance: Number(form.balance) || 0, is_cash_account: form.is_cash_account,
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          ...
          <div style={{ background: '#fff', borderRadius: '0.875rem', padding: '2rem', width: '100%', maxWidth: 480, boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1.5rem' }}>{editItem ? 'Edit Akun' : 'Tambah Akun'}</h2>
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Kode *</label>
                  <input required type="text" placeholder="1-1100" value={form.code} onChange={(e) => setForm(f => ({ ...f, code: e.target.value }))}
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Nama Akun *</label>
                <input required type="text" placeholder="Nama lengkap akun" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))}
                  style={{ width: '100%', padding: '0.625rem', border: '1px solid #d1d5db', borderRadius: '0.5rem', fontSize: '0.875rem', outline: 'none' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
