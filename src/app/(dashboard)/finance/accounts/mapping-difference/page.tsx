...
export default function MappingDifferencePage() {
  ...
  const [form, setForm] = useState({
    transaction_type: 'exchange_rate_diff', debit_account_id: '', credit_account_id: '', description: '',
  })
            ...
            <tbody>
              {mappings.map((m) => (
                <tr key={m.id}>
                  ...
                  <td>{m.debit_account ? <span style={{ fontFamily: 'monospace' }}>{m.debit_account.code}</span> : '—'} {m.debit_account?.name ?? ''}</td>
                  <td>{m.credit_account ? <span style={{ fontFamily: 'monospace' }}>{m.credit_account.code}</span> : '—'} {m.credit_account?.name ?? ''}</td>
          ...
          <div style={{ background: '#fff', borderRadius: '0.875rem', padding: '2rem', width: '100%', maxWidth: 480, boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Akun Debit (Selisih Masuk)</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#374151', marginBottom: '0.3rem' }}>Akun Kredit (Selisih Keluar)</label>
