...
export default function AdminReportsPage() {
		...
		if (month !== 0) {
			...
			prevFiltered =
				(data as Order[])?.filter(
					(o) => new Date(o.created_at).getFullYear() === year - 1,
				) ?? [];
		}
	...
	orders.forEach((o) => {
		(o.order_items ?? []).forEach((item) => {
			...
			if (!productRevenue[name])
				productRevenue[name] = { name, count: 0, revenue: 0 };
	...
	function exportPDF() {
		const doc = new jsPDF();
		const periodLabel =
			month === 0 ? `Tahun ${year}` : `${MONTHS[month - 1]} ${year}`;
		doc.setFontSize(16);
		...
		doc.text(
			`Periode: ${periodLabel} — Generated: ${new Date().toLocaleDateString("id-ID")}`,
			14,
			28,
		);
			...
			>
				<div>
					...
					<p className="page-subtitle">
						Laporan penjualan dan pipeline pesanan
					</p>
				</div>
