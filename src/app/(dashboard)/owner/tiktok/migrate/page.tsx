...
export default function RunMigrationPage() {
  ...
  async function checkAndRun() {
    ...
    let existing = null
    try {
      const result = await supabase.from('tiktok_shop_settings').select('id').limit(1).maybeSingle()
    ...
    for (const sql of statements) {
      try {
        ...
        if (error) {
          ...
          setStatus((s) => [...s, `⚠️ exec_sql not available: ${error.message.slice(0, 80)}`])
          setStatus((s) => [...s, 'ℹ️ Please run migration manually via Supabase SQL Editor (see instructions below)'])
          setRunning(false)
          return
        }
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 640, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '1rem' }}>TikTok Shop Migration</h1>
      ...
      {!running && status.some((s) => s.includes('Please run migration manually')) && (
        <div
          style={{
            marginTop: '1.5rem',
            padding: '1rem',
