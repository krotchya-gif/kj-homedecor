CREATE OR REPLACE FUNCTION public.create_journal_tx(
...
BEGIN
  ...
  END IF;
  INSERT INTO public.journal_entries (entry_date, description, reference_type, reference_id, total_debit, total_credit, is_auto, created_by, idempotency_key)
  VALUES (p_entry_date, p_description, p_reference_type, p_reference_id, v_total_debit, v_total_credit, COALESCE(p_is_auto, false), p_created_by, p_idempotency_key)
  RETURNING id INTO v_entry_id;
  INSERT INTO public.journal_lines (entry_id, account_id, debit, credit, description)
  SELECT v_entry_id, (x.record).account_id, COALESCE((x.record).debit, 0), COALESCE((x.record).credit, 0), COALESCE((x.record).description, NULL)
...
END;
$$;
REVOKE ALL ON FUNCTION public.create_journal_tx FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_journal_tx FROM anon;
GRANT EXECUTE ON FUNCTION public.create_journal_tx TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_journal_tx TO service_role;
